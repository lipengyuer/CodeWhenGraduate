classdef ActFunc < handle
    ...
    methods (Static = true)
        ...
        function [ cur_grads ] = ...
            ...
            %   cur_grads: gradients at current layer (obs_dim x cur_dim)
            %
            exp_vals = exp(pre_acts * pre_weights);%计算经过传递之后的值，得到的值是个向量
            sm_vals = bsxfun(@rdivide, exp_vals, sum(exp_vals,2));%在此处计算出了向量中各个元素的百分比
            sm_grads = sm_vals .* (1 - sm_vals);%计算出了求导以后的值
            cur_grads = sm_grads .* (post_grads * post_weights');%计算出了新的梯度值
            return
        end
