function [WW alldata_cha_train xuyaodeshuxing]=tezhengxuanze_relief_guiyihua_maxmin_changedsamples_tezhengfenx(cishu)
% clear;
% cishu=30;
...
data_cipin_guiyihua=data_cipin_guiyihua';
guiyihuamoban_cipin=guiyihuamoban_cipin';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%词频
...
data_bqfh_guiyihua=data_bqfh_guiyihua';
guiyihuamoban_bqfh=guiyihuamoban_bqfh';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
save C:\Users\admini\Desktop\aaaaa\depression\选择特征时的分析数据.mat data_cipin data_cipin_  weizhi_cipinnotzero weizhi_cipinnotzero weizhi_bqfhnozero
%%%%%%%%%%%%%发微时间%%%
fawei_num=zhenheji';
...
data_fawei_guiyihua=data_fawei_guiyihua';
guiyihuamoban_fawei=guiyihuamoban_fawei';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
data_Eco_guiyihua=data_Eco_guiyihua';
guiyihuamoban_Eco = guiyihuamoban_Eco';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
save C:\Users\admini\Desktop\aaaaa\depression\抽样训练集测试集.mat alldata_cha_train alldata_cha_test
load C:\Users\admini\Desktop\aaaaa\depression\抽样训练集测试集.mat
%%%%%%%%%%%%%%%%
weightoffeatures=[];
for N=1:30
[Woffeatures WW]= Relief([alldata_cha_train(:,1:end-1)],50,1);
weightoffeatures=[weightoffeatures;Woffeatures];
fprintf('第%d次循环选择操作\n',i);
end;
save C:\Users\admini\Desktop\aaaaa\depression\特征权重数据.mat weightoffeatures
load C:\Users\admini\Desktop\aaaaa\depression\特征权重数据.mat
%%%%%%%%%%
ordernum=[1:size(alldata_cha_train,2)-2]';
% aftersorting=-sortrows(-[abs(Woffeatures(end,:)') ordernum],1);
aftersorting=sortrows([sum(weightoffeatures)' ordernum],1);
xuyaodeshuxing=aftersorting(:,2);
% xuyaodeshuxing=ordernum;
numoffeatures=cishu;%特征维数
alldata=[ alldata_cha_train(:,xuyaodeshuxing(1:end)) alldata_cha_train(:,end-1:end)];
save 'C:\Users\admini\Desktop\woca11.mat' alldata%保存测试集
alldata_test=[ alldata_cha_test(:,xuyaodeshuxing(1:end)) alldata_cha_test(:,end-1:end)];
save 'C:\Users\admini\Desktop\data4test.mat' alldata_test %保存测试集
save 'C:\Users\admini\Desktop\数据和权重.mat' alldata alldata_test weightoffeatures%保存测试集
