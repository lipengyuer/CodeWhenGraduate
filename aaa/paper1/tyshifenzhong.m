function [jianyanzhi,hejizhuanfafenbu,heji,zhenheji,zhenzhenheji]=tyshifenzhong(x)
year=x(:,1);
month=x(:,2);
day=x(:,3);
q=length(year); 
...
zhuanfafenbu1=zeros(shijianduanshu,tianshu);
for a=1:tianshu
    for m=0:(shijianduanshu-1)
        
           for b=1:yitianzhuanfashu(a)
            if ((xx(b,a)==rem(m,60)*0.01+fix(m/60))&&xx(b,a)<(rem(m,60)*0.01+fix(m/60)+0.01));%实现60进制的一个办法
             zhuanfafenbu(m+1,a)=zhuanfafenbu(m+1,a)+1;
             zhuanfafenbu1(m+1,a)=rem(m,60)*0.01+fix(m/60);
            end
           end
