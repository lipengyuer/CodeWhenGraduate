function [jianyanzhi,hejizhuanfafenbu]=tyhalf(x)
year=x(:,1);
month=x(:,2);
day=x(:,3);
q=length(year); 
...
for a=1:tianshu
    for m=0:(shijianduanshu-1)
           ...
           for b=1:yitianzhuanfashu(a)
            if ((xx(b,a)>=rem(m,yixiaoshiduanshu)*0.3+fix(m/yixiaoshiduanshu))&&(xx(b,a)<0.3+rem(m,yixiaoshiduanshu)*0.3+fix(m/yixiaoshiduanshu)));
