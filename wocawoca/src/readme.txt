...
* If the MCR is not installed, run MCRInstaller, located in:
  ...
  D:\filesss\MATLAB\toolbox\compiler\deploy\win64\MCRInstaller.exe
...
* Ensure you have the version (1.6.0) of the Java Runtime Environment (JRE). See section 
  5A.
* wocawoca.jar must be included in your CLASSPATH.
...
-MCRInstaller.exe 
   - include when building component by selecting "include MCR" option 
     in deploytool
-Javadoc   
   - javadoc for wocawoca is in the doc directory. While  
     ...
     ii. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by opening 
        ...
        win32 or win64:
            ...
            set PATH=<mcr_root>\v711\runtime\{win32|win64};%PATH% 

        Alternately, for Windows, add the following pathname:
            <mcr_root>\v711\runtime\{win32|win64}
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        Solaris64
            setenv LD_LIBRARY_PATH /usr/lib/lwp:
                <mcr_root>/v711/runtime/sol64:
                <mcr_root>/v711/sys/os/sol64:
                <mcr_root>/v711/sys/java/jre/sol64/jre/lib/sparcv9/native_threads:
                <mcr_root>/v711/sys/java/jre/sol64/jre/lib/sparcv9/server:
                <mcr_root>/v711/sys/java/jre/sol64/jre/lib/sparcv9
            setenv XAPPLRESDIR <mcr_root>/v711/X11/app-defaults
        ...
        Maci
            setenv DYLD_LIBRARY_PATH
                <mcr_root>/v711/runtime/maci:
                <mcr_root>/v711/sys/os/maci:
                <mcr_root>/v711/bin/maci:
