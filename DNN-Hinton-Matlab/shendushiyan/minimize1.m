function [X, fX, i] = minimize1(X, f, length, varargin)%varargin代表Dim等f法人输入？？？真的是这样
...
[f0 df0] = feval(f, X, varargin{:});          % get function value and gradient%计算方程值和梯度
fX = f0;
i = i + (length<0);                                            % count epochs?!
s = -df0; d0 = -s'*s;           % initial search direction (steepest) and slope%???????????????????????????????d0是什么东西，负的梯度的模的平方
x3 = red/(1-d0);                                  % initial step is red/(|s|+1)%为什么是标签维数除以这个数？

while i < abs(length)                                      % while not finished循环知道i等于length的绝对值，这个length等于784？
  ...
  while 1                             % keep extrapolating as long as necessary
    ...
    while ~success && M > 0
      try
        ...
        [f3 df3] = feval(f, X+x3*s, varargin{:});%用旧值算新的梯度和方程值
        if isnan(f3) || isinf(f3) || any(isnan(df3)+isinf(df3)), error(''), end%isnan是什么：将f3中的nan换为1，其他换为0。isinf将inf换为1，其他0
        success = 1;
      catch                                % catch any error which occured in f
        x3 = (x2+x3)/2;                                  % bisect and try again
      end
    end
    if f3 < F0, X0 = X+x3*s; F0 = f3; dF0 = df3; end         % keep best values%若新方程值小于旧值，更替方程值和梯度？？if后边的逗号是什么意思
    d3 = df3'*s;                                                    % new slope
